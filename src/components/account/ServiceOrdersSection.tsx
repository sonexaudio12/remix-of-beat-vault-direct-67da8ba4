...
function ServiceOrderCard({ order }: { order: ServiceOrder }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [filesRes, updatesRes] = await Promise.all([
        supabase.from('service_order_files').select('id, file_name, file_size').eq('order_id', order.id),
          ...
          {files.length > 0 && (
            <div>
              ...
              <div className="space-y-1">
                {files.map(f => (
                  <div key={f.id} className="flex items-center gap-2 p-2 rounded bg-secondary/50 text-sm">
          ...
          {updates.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {updates.map(u => (
                  <div key={u.id} className="text-sm p-3 rounded-lg bg-secondary/50">
