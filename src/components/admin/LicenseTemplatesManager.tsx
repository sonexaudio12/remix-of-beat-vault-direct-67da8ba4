...
const PLACEHOLDERS = [
  { key: '{CONTRACT_DATE}', desc: 'Date of purchase' },
  { key: '{CURRENT_YEAR}', desc: 'Current year' },
  { key: '{CUSTOMER_FULLNAME}', desc: 'Customer name' },
  { key: '{CUSTOMER_EMAIL}', desc: 'Customer email' },
  { key: '{CUSTOMER_ADDRESS}', desc: 'Customer address (optional)' },
  { key: '{PRODUCT_TITLE}', desc: 'Beat / kit title' },
  { key: '{LICENSE_NAME}', desc: 'License tier name' },
  { key: '{FILE_TYPE}', desc: 'MP3 / WAV / STEMS' },
  { key: '{PRODUCT_PRICE}', desc: 'Purchase price' },
  { key: '{ORDER_ID}', desc: 'Order reference' },
  { key: '{PRODUCT_OWNER_FULLNAME}', desc: 'Producer legal name (from Settings)' },
  { key: '{PRODUCER_ALIAS}', desc: 'Producer brand name (from Settings)' },
  { key: '{STATE_PROVINCE_COUNTRY}', desc: 'Governing law (from Settings)' },
];
    ...
    <div className="space-y-6">
      <div className="rounded-xl bg-card border border-border p-6">
        ...
        <p className="text-muted-foreground text-sm mb-6">
          Upload PDF license templates for each tier. Use placeholders like <code className="text-xs bg-secondary px-1.5 py-0.5 rounded">{'{CUSTOMER_FULLNAME}'}</code> in your PDFs — they'll be automatically replaced with real data after each purchase.
        </p>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {PLACEHOLDERS.map(({ key, desc }) => (
            <div key={key} className="flex items-center gap-3 py-1.5">
              <code className="text-xs bg-secondary px-2 py-1 rounded font-mono shrink-0">{key}</code>
