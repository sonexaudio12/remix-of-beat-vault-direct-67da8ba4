...
const SECTION_SETTING_LABELS: Record<string, Record<string, {
  label: string;
...
export function SectionEditor() {
  ...
  return <div className="max-w-3xl text-secondary-foreground bg-popover">
      ...
      {showTemplates && <div className="grid grid-cols-2 gap-3 mb-6">
          {SECTION_TEMPLATES.map(t => <button key={t.id} onClick={() => applyTemplate(t.id)} className="text-left rounded-xl border border-border bg-card p-4 hover:border-primary hover:shadow-md transition-all">
      ...
      <div className="space-y-2">
        ...
        return <div key={section.id} draggable onDragStart={() => handleDragStart(idx)} onDragOver={e => handleDragOver(e, idx)} onDragEnd={handleDragEnd} className={`rounded-xl border bg-card transition-all ${dragIdx === idx ? 'border-primary shadow-lg scale-[1.02]' : 'border-border'} ${!section.enabled ? 'opacity-60' : ''}`}>
              {/* Header row */}
              <div className="flex items-center gap-3 p-4 cursor-grab active:cursor-grabbing">
                <GripVertical className="h-5 w-5 text-muted-foreground flex-shrink-0" />

                <button onClick={() => setExpandedId(isExpanded ? null : section.id)} className="flex items-center gap-2 flex-1 text-left">
                ...
                <div className="flex items-center gap-2">
                  {section.enabled ? <Eye className="h-4 w-4 text-primary" /> : <EyeOff className="h-4 w-4 text-muted-foreground" />}
                  <Switch checked={section.enabled} onCheckedChange={() => toggleEnabled(section.id)} />
                </div>
              </div>

              {/* Expanded settings */}
              {isExpanded && Object.keys(settingsDef).length > 0 && <div className="px-4 pb-4 pt-2 border-t border-border space-y-4">
                  {Object.entries(settingsDef).map(([key, def]) => <div key={key}>
                      <Label className="text-sm mb-1 block">{def.label}</Label>
                      {def.type === 'textarea' ? <Textarea value={section.settings[key] || ''} onChange={e => updateSetting(section.id, key, e.target.value)} rows={3} /> : <Input type={def.type === 'number' ? 'number' : 'text'} value={section.settings[key] || ''} onChange={e => updateSetting(section.id, key, e.target.value)} />}
                    </div>)}
                </div>}
            </div>;
      })}
