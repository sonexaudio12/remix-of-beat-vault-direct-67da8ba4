...
export function ServiceOrdersManager() {
  ...
  return <div className="space-y-4">
      ...
      {orders.length === 0 ? <div className="text-center py-12 text-muted-foreground">No service orders yet.</div> : <div className="space-y-3">
          {orders.map(order => <ServiceOrderRow key={order.id} order={order} onUpdate={fetchOrders} />)}
        </div>}
    </div>;
}
function ServiceOrderRow({
  ...
  const loadDetails = async () => {
    const [filesRes, updatesRes] = await Promise.all([supabase.from('service_order_files').select('*').eq('order_id', order.id), supabase.from('service_order_updates').select('*').eq('order_id', order.id).order('created_at', {
      ascending: false
    })]);
  ...
  const handleDownload = async (filePath: string, fileName: string) => {
    const {
      data,
      error
    } = await supabase.storage.from('service-files').createSignedUrl(filePath, 3600);
  ...
  return <div className="rounded-xl border border-border bg-card">
      <button onClick={() => setExpanded(!expanded)} className="w-full p-4 flex items-center justify-between text-left text-secondary-foreground bg-popover">
      ...
      {expanded && <div className="border-t border-border p-4 space-y-4 bg-popover text-secondary-foreground">
          ...
          {files.length > 0 && <div>
              ...
              <div className="space-y-1">
                {files.map(f => <div key={f.id} className="flex items-center justify-between p-2 rounded bg-secondary/50 text-sm">
          ...
          {updates.length > 0 && <div>
              ...
              <div className="space-y-2">
                {updates.map(u => <div key={u.id} className="text-sm p-2 rounded bg-secondary/50">
