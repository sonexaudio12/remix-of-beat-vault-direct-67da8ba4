  ...
  return <>
      <div className="beat-card group border border-border/50">
        ...
        <div className="relative aspect-square overflow-hidden">
          <img src={beat.coverUrl} alt={beat.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" />
          
          {/* Play Button Overlay */}
          <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-300 group-hover:opacity-100 text-destructive bg-[#c8d3df]/[0.28]">
            <Button variant="player" size="iconLg" onClick={handlePlay} className="transform scale-90 group-hover:scale-100 transition-transform">
              {isCurrentlyPlaying ? <Pause className="h-6 w-6" /> : <Play className="h-6 w-6 ml-0.5" />}
            </Button>
          </div>

          {/* Now Playing Indicator */}
          {isCurrentlyPlaying && <div className="absolute bottom-3 left-3 flex items-center gap-1.5 rounded-full bg-primary/90 px-3 py-1.5">
              <div className="flex gap-0.5">
                {[...Array(3)].map((_, i) => <div key={i} className="w-0.5 bg-primary-foreground rounded-full animate-waveform" style={{
              height: '12px',
