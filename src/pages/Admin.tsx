  ...
  return <div className="space-y-4 max-w-3xl">
      ...
      <div className="rounded-xl border border-border overflow-hidden">
        <button onClick={() => toggle('contact')} className="w-full flex items-center justify-between px-6 py-4 bg-card hover:bg-muted/30 transition-colors">
          ...
          <ChevronDown className={`h-5 w-5 text-muted-foreground transition-transform ${openSection === 'contact' ? 'rotate-180' : ''}`} />
        </button>
        {openSection === 'contact' && <div className="px-2 pb-2"><ContactSettingsManager /></div>}
      </div>

      {/* PayPal Configuration */}
      <div className="rounded-xl border border-border overflow-hidden">
        <button onClick={() => toggle('paypal')} className="w-full flex items-center justify-between px-6 py-4 bg-card hover:bg-muted/30 transition-colors">
          <span className="font-semibold">PayPal Configuration</span>
          <ChevronDown className={`h-5 w-5 text-muted-foreground transition-transform ${openSection === 'paypal' ? 'rotate-180' : ''}`} />
        </button>
