...
function DashboardContent({ isAdmin, setActiveTab }: { isAdmin: boolean; setActiveTab: (tab: string) => void }) {
  ...
  const statItems = [
    ...
    { label: 'Beats Sold', value: stats.beatsSold.toString(), icon: Music2 },
    { label: 'Total Beats', value: stats.totalBeats.toString(), icon: Package },
    { label: 'Pending Orders', value: stats.pendingOrders.toString(), icon: FileText },
  ];
...
function UploadContent({ isAdmin, onSuccess }: { isAdmin: boolean; onSuccess?: () => void }) {
  if (!isAdmin) {
    return (
    ...
    <div className="rounded-xl bg-card border border-border p-6">
      ...
      <p className="text-muted-foreground text-sm mb-4">
        {isAdmin 
          ...
          : 'Admin access required to manage license templates.'}
      </p>
      {isAdmin && (
        <div className="grid gap-4 md:grid-cols-2">
          {['MP3 Lease', 'WAV Lease', 'Trackout (Stems)', 'Exclusive'].map((type) => (
